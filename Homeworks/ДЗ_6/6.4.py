    ...
    with open('users.csv', encoding='utf-8') as users:
        with open('hobby.csv', encoding='utf-8') as hobby:
            ...
            for line_user, line_hobby in zip_longest(users, hobby):
                ...
                '''Данная часть была сделана по заданию, выложенному на сайте
                ("Также реализовать парсинг данных из файлов — получить отдельно фамилию,
                имя и отчество для пользователей и название каждого хобби".)Уже после, когда столкнулся с
                затруднениями и полез смотреть разбор, понял, что разбирали другое :D'''
